...
export const getInformationCustomer = (data) => ({
  type: types.CUSTOMER,
  payload: data,
})
...
//!Login Google
export const loginGoogleInitiate = (tokenId) => {
  return async function (dispatch) {
    // dispatch(LoginGoogleStart());
    await axios
      .post(LOGIN_GOOGLE, { tokenId: tokenId })
      .then((data) => {
        dispatch(getInformationCustomer(data.data.user));
